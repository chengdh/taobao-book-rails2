class BaseItem < ActiveRecord::Base
  ...
  def self.get_notify_items(sess)
    ...
    if start_modified <  6.days.ago.beginning_of_day.strftime('%Y-%m-%d %H:%M:%S') 
      start_modified = 6.days.ago.beginning_of_day.strftime('%Y-%m-%d %H:%M:%S')
      last_syn_time = 6.days.ago.beginning_of_day
    end
  ...
  def save2taobao(sess,options = {})
    ...
    ::Rails.logger.debug remote_item
